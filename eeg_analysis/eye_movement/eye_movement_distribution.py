...
def resolve_event_summary_path(input_path: Path) -> Path:
    ...
    raise FileNotFoundError(
        "Input must be an eye_movement_validation output folder or a direct event_summary.csv path."
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot eye movement peak-amplitude distributions from eye_movement_validation output. "
    ...
    parser.add_argument(
        "input_path",
        nargs="?",
        help="eye_movement_validation output folder or direct event_summary.csv path.",
    )
    parser.add_argument(
        "--output",
...
def main() -> int:
    ...
    if not raw_input_path:
        raw_input_path = input("請輸入 eye_movement_validation 輸出資料夾或 event_summary.csv 路徑: ")
