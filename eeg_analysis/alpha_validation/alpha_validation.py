...
from eeg_analysis.alpha_validation.alpha_detection_distribution import (
    save_alpha_energy_normal_distribution,
)
from eeg_analysis.alpha_validation.alpha_detection_paths import (
    default_plot_dir,
    normalize_user_path,
    resolve_input_paths,
)
from eeg_analysis.alpha_validation.alpha_detection_plotting import export_classified_plots
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--notch-freq",
        type=float,
        help="Apply a zero-phase notch filter at this frequency before FFT and plotting. Use 60 to suppress mains noise.",
    )
...
def main() -> int:
    ...
    if bool(args.highpass):
        signal = apply_highpass_filter(signal, fs, cutoff_hz=1)
    if bool(args.lowpass):
        signal = apply_lowpass_filter(signal, fs, cutoff_hz=30.0)
    # if args.notch_freq is not None:
    #     signal = apply_notch_filter(signal, fs, notch_freq=float(args.notch_freq))
    window = int(round(fs))
    if window <= 0:
    ...
    print(f"fs: {fs}")
    print(f"highpass: {bool(args.highpass)}")
    print(f"notch_freq: {args.notch_freq if args.notch_freq is not None else 'off'}")
    print(f"lowpass: {bool(args.lowpass)}")
